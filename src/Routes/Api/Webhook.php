...
final class Webhook {
    ...
    public function handle(WP_REST_Request $request): WP_REST_Response {
        ...
        if (!$this->allowByRateLimit($ip)) {
            ...
            return new WP_REST_Response(['code' => 'rate_limited', 'message' => __('Too many requests.', 'notification-hub')], 429);
        }
        ...
        if (!is_array($body)) {
            ...
            return new WP_REST_Response(['code' => 'invalid_json', 'message' => __('Invalid JSON body.', 'notification-hub')], 400);
        }
        ...
        if ($id <= 0) {
            ...
            return new WP_REST_Response(['code' => 'insert_failed', 'message' => __('Insert failed.', 'notification-hub')], 400);
        }
    ...
    private function verifySignature(WP_REST_Request $request, string $ip) {
        ...
        if ($timestampRaw === '' || $signatureRaw === '') {
            ...
            return new WP_REST_Response(['code' => 'missing_signature', 'message' => __('Missing signature headers.', 'notification-hub')], 401);
        }
        ...
        if (!ctype_digit($timestampRaw)) {
            ...
            return new WP_REST_Response(['code' => 'invalid_timestamp', 'message' => __('Invalid timestamp.', 'notification-hub')], 401);
        }
        ...
        if (abs(time() - $timestamp) > $window) {
            ...
            return new WP_REST_Response(['code' => 'timestamp_expired', 'message' => __('Timestamp expired.', 'notification-hub')], 401);
        }
        ...
        if (count($sigParts) !== 2 || strtolower((string) $sigParts[0]) !== 'sha256') {
            ...
            return new WP_REST_Response(['code' => 'invalid_signature_format', 'message' => __('Invalid signature format.', 'notification-hub')], 401);
        }
        ...
        if ($givenHex === '' || !ctype_xdigit($givenHex)) {
            ...
            return new WP_REST_Response(['code' => 'invalid_signature', 'message' => __('Invalid signature.', 'notification-hub')], 401);
        }
        ...
        if (!hash_equals($expectedHex, $givenHex)) {
            ...
            return new WP_REST_Response(['code' => 'invalid_signature', 'message' => __('Unauthorized.', 'notification-hub')], 401);
        }
        ...
        if (get_transient($replayKey) !== false) {
            ...
            return new WP_REST_Response(['code' => 'replay_detected', 'message' => __('Replay detected.', 'notification-hub')], 401);
        }
