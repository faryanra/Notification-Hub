...
final class LoadNotification {
    ...
    public function handle(): void {
        ...
        $email = $templates->render('notifications/email.php', [
            ...
            'source_human' => (string) ($payload['source_human'] ?? ''),
            'type_human' => (string) ($payload['type_human'] ?? ''),
        ]);
        if ($email === '') {
            $email = (string) ($payload['summary'] ?? '');
        }

        $telegram = $templates->render('notifications/telegram.php', [
            'title' => (string) ($payload['title'] ?? ''),
            'summary' => (string) ($payload['summary'] ?? ''),
            'link' => (string) ($payload['link'] ?? ''),
            'context' => isset($payload['context']) && is_array($payload['context']) ? $payload['context'] : [],
        ...
        if ($telegram === '') {
            $telegram = (string) ($payload['summary'] ?? '');
        }

        $slack = $templates->render('notifications/slack.php', [
            'title' => (string) ($payload['title'] ?? ''),
            'summary' => (string) ($payload['summary'] ?? ''),
            'link' => (string) ($payload['link'] ?? ''),
            'context' => isset($payload['context']) && is_array($payload['context']) ? $payload['context'] : [],
