...
use NotificationHub\Presenters\Admin\AnalyticsPage;
use NotificationHub\Presenters\Admin\HooksPage;
use NotificationHub\Presenters\Admin\RulesPage;
...
final class MenuRegistration implements Integration {
     ...
     * - sub: nh-dashboard, nh-hooks, nh-rules, nh-analytics, nh_settings
     *
     * @since 1.0.0
     * @return void
     */
    public function registerMenus(): void {
        ...
        add_submenu_page(
            'nh-dashboard',
            esc_html__('Rules', 'notification-hub'),
            esc_html__('Rules', 'notification-hub'),
            'manage_options',
            'nh-rules',
            [$this, 'renderRules']
        ...
        add_submenu_page(
            'nh-dashboard',
            esc_html__('Analytics', 'notification-hub'),
            esc_html__('Analytics', 'notification-hub'),
            'manage_options',
            'nh-analytics',
    ...
    public function renderRules(): void {
        $this->ensurePresenterLoaded(RulesPage::class, 'Presenters/Admin/RulesPage.php');

        if (class_exists(RulesPage::class)) {
            (new RulesPage())->render();
            return;
        }

        echo '<div class="wrap"><h1>' . esc_html__('Rules', 'notification-hub') . '</h1><p>' . esc_html__('Presenter is missing.', 'notification-hub') . '</p></div>';
    }
    ...
    public function renderAnalytics(): void {
        $this->ensurePresenterLoaded(AnalyticsPage::class, 'Presenters/Admin/AnalyticsPage.php');

        if (class_exists(AnalyticsPage::class)) {
            (new AnalyticsPage())->render();
            return;
        }

        echo '<div class="wrap"><h1>' . esc_html__('Analytics', 'notification-hub') . '</h1><p>' . esc_html__('Presenter is missing.', 'notification-hub') . '</p></div>';
    }
