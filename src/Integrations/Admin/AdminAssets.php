...
final class AdminAssets implements Integration {
    ...
    public function enqueue(string $hook): void {
        ...
        $is_rules     = (strpos($hook, 'nh-rules') !== false);
        $is_analytics = (strpos($hook, 'nh-analytics') !== false);
        $is_settings  = (strpos($hook, 'nh_settings') !== false);

        if (!$is_dashboard && !$is_hooks && !$is_rules && !$is_analytics && !$is_settings) {
            return;
        }
        ...
        if (is_rtl()) {
            wp_enqueue_style('nh-admin-rtl', NH_PLUGIN_URL . 'assets/css/admin-rtl.css', ['nh-admin'], NH_VERSION);
        }
        ...
        if ($is_analytics) {
            wp_enqueue_style('nh-analytics', NH_PLUGIN_URL . 'assets/css/admin/analytics.css', $style_deps, NH_VERSION . '-analytics', 'all');
        }
    }
}
