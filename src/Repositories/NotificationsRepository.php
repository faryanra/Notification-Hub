...
use NotificationHub\Services\AutomationEngine;
...
final class NotificationsRepository {
    ...
    public function insert(array $e): int {
        ...
        if (!(new SettingsRepository())->isEventEnabled($source, $type)) {
            return 0;
        }
        ...
        if ($inserted_id > 0) {
            ...
            $link = isset($e['link']) ? esc_url_raw((string) $e['link']) : '';
            if ($link === '' && isset($decodedContext['admin_link']) && is_scalar($decodedContext['admin_link'])) {
                $link = esc_url_raw((string) $decodedContext['admin_link']);
            }
            ...
            $already_dispatched_channels = $this->dispatchChannels(
                [
                    'source'  => $source,
                    'type'    => $type,
                    'title'   => $title,
            ...
            // MVP automation: evaluate rules only on insert to avoid update loops.
            (new AutomationEngine())->processOnInsert($inserted_id, $normalized, $already_dispatched_channels);
        }
     ...
     * @return array<int,string> Channels already dispatched by global settings flow.
     */
    private function dispatchChannels(array $notification, array $rawInput): array {
        ...
        $channelsSettings = $settings->getChannels();
        ...
        if (!empty($channelsSettings['telegram_bot_token']) && !empty($channelsSettings['telegram_chat_id'])) {
            $channels[] = 'telegram';
        }

        if (!empty($channelsSettings['slack_webhook'])) {
            $channels[] = 'slack';
        }
        ...
        $dispatched = array_values(array_unique($channels));
    ...
    public function markArchived(int $id): bool {
        global $wpdb;
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
        $updated = $wpdb->update($this->table(), ['status' => 1], ['id' => $id], ['%d'], ['%d']);
        return ($updated !== false) && empty($wpdb->last_error);
    }
    ...
    public function unarchive(int $id): bool {
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
        $updated = $wpdb->update($this->table(), ['status' => 0], ['id' => $id, 'status' => 1], ['%d'], ['%d', '%d']);
        return ($updated !== false) && empty($wpdb->last_error);
    }
