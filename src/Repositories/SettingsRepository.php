...
final class SettingsRepository {
    ...
    public const OPT_ENABLED_EVENTS = 'nh_enabled_events';
    ...
    public function getEnabledEvents(): array {
        $raw = get_option(self::OPT_ENABLED_EVENTS, EventCatalog::defaultKeys());
        return self::sanitizeEnabledEvents($raw);
    }

    public function isEventEnabled(string $source, string $type): bool {
        $eventKey = EventCatalog::keyFor($source, $type);
        if (!EventCatalog::isKnownKey($eventKey)) {
            return true;
        }

        $enabled = $this->getEnabledEvents();
        return in_array($eventKey, $enabled, true);
    }
     ...
     * @return array<int,string>
     */
    public static function sanitizeEnabledEvents($value): array {
        return EventCatalog::sanitizeKeys($value);
    }
     ...
     */
    public function updateChannels(array $data): void {
