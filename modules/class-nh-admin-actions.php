...
// =====================================================
// Export Notifications to CSV (with priority & tags)
// =====================================================
add_action('admin_post_nh_export_csv', function() {
    ...
    // Base export columns — only those that actually exist
    ...
    foreach ($rows as $r) {
        // Handle optional tags or context
        if (isset($r['tags'])) {
            $r['tags'] = $r['tags'] ? implode('|', (array) json_decode($r['tags'], true)) : '';
        }
        if (isset($r['context']) && is_string($r['context']) && str_starts_with(trim($r['context']), '{')) {
            $r['context'] = json_encode(json_decode($r['context'], true), JSON_UNESCAPED_UNICODE);
        }
        fputcsv($out, $r);
    }
...
// =====================================================
// AJAX — Notification Status Updates (v1.6.0)
// =====================================================

function nh_ajax_handle($action, callable $callback) {
    add_action("wp_ajax_{$action}", function() use ($callback) {
        ...
        $result = $callback($wpdb, $table, $id);

        if ($wpdb->last_error) {
            wp_send_json_error(['message' => $wpdb->last_error], 500);
        }

        wp_send_json_success(['id' => $id, 'status' => $result]);
    });
}

// MARK READ
nh_ajax_handle('nh_mark_read', function($wpdb, $table, $id) {
    $wpdb->update($table, ['read_at' => current_time('mysql')], ['id' => $id]);
    return 'read';
});

// MARK UNREAD
nh_ajax_handle('nh_mark_unread', function($wpdb, $table, $id) {
    $wpdb->update($table, ['read_at' => null], ['id' => $id]);
    return 'unread';
});

// MARK IMPORTANT (status=3)
nh_ajax_handle('nh_mark_important', function($wpdb, $table, $id) {
    $wpdb->update($table, ['status' => 3], ['id' => $id]);
    return 'important';
});

// UNMARK IMPORTANT (status=0)
nh_ajax_handle('nh_unmark_important', function($wpdb, $table, $id) {
    $wpdb->update($table, ['status' => 0], ['id' => $id]);
