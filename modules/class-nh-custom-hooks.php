...
class NH_Custom_Hooks {
    ...
    public function save_hook(): void {
        ...
        $exists = (int) $wpdb->get_var(
            $wpdb->prepare("SELECT COUNT(*) FROM {$table} WHERE action_name=%s", $action)
        );
        ...
        wp_safe_redirect(add_query_arg(['page' => 'nh-hooks', 'saved' => 1], admin_url('admin.php')));
        exit;
    }
    ...
    public function test_hook(): void {
        ...
        wp_safe_redirect(add_query_arg(['page' => 'nh-hooks', 'tested' => 1], admin_url('admin.php')));
        exit;
    }
