// Notification Hub â€” Dashboard (live refresh + actions + filters)
//
...
(function () {
  ...
  const t = (key, fallback) => (window.nh_i18n && window.nh_i18n[key]) || fallback;
  ...
  document.body.addEventListener('click', (e) => {
    ...
    fetch(
      `${window.nhAdmin.ajax_url}?action=nh_view_notification&id=${encodeURIComponent(id)}&_wpnonce=${encodeURIComponent(nonce)}`,
      ...
      .then((data) => {
        if (data.success) {
          ...
          const title =
            payload?.title || data?.data?.source || t('modal_default_title', 'Notification');

          const channel = localStorage.getItem('nh_preview_channel') || 'email';
          ...
          document.getElementById('nh-modal-meta').textContent = data.data.created_at || '';
          ...
          if (metaEl && !metaEl.querySelector('.nh-preview-switch')) {
            ...
            select.addEventListener('change', () => {
              ...
              const next = preview?.[c] || preview?.email || preview?.telegram || preview?.slack || '';
  ...
  // Live refresh (server-render swap)
    ...
    return fetch(window.location.href, { credentials: 'same-origin' })
      ...
      .then((html) => {
        ...
        tableBody.querySelectorAll('tr').forEach((tr) => {
          const did =
            tr.getAttribute('data-id') || tr.querySelector('td:nth-child(2)')?.textContent.trim() || '';
