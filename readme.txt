...
Notification Hub centralizes all WordPress notifications into one unified dashboard — now smarter, cleaner, and more reliable than ever.
...
**New in v1.5.1 — Stability & UX Refinement**
- Fixed: “Mark as Read” (single + AJAX) now works reliably
- Fixed: Badge count and live refresh logic (no premature reset)
- Fixed: Highlighted rows stay visible until manual reload or read
- Fixed: CSV Export blank page bug
- Improved: JS modular structure (`admin.js` + `dashboard.js`)
- Improved: Dashboard count accuracy (All / Active / Archived)
- Improved: PHPDoc + English code comments for all modules
- Cleaned: Duplicated listeners, redundant WP_List_Table overrides

== Features ==
- 📬 Centralized notification dashboard (All / Active / Archived)
- 🔔 Global admin bar unread badge (live AJAX refresh)
- ✉️ Email notifications (core)
- 💬 Telegram & Slack (Pro)
- 🧩 Custom Hooks Manager (CRUD + Test)
- 🔧 REST & Webhook APIs
- 📊 CSV Export for filtered view
- 🧠 Real-time refresh + highlight persistence
...
= 1.5.1 — Stability & UX Refinement =
* Fixed: “Mark as Read” AJAX endpoint and nonce handling
* Fixed: CSV Export blank page bug
* Fixed: Highlight logic (new rows persist until manually read)
* Fixed: Badge refresh logic (no auto-reset when inside dashboard)
* Improved: Dashboard table PHP code (clarity + comments)
* Improved: `admin.js` rewritten with no duplicate listeners
* Improved: Security checks and nonce consistency
* Cleanup: Removed redundant event bindings + inline JS
* Cleanup: Fully documented PHP files 
